...
- **RovoDev-first mode**: `pnpm run rovodev` starts RovoDev Serve (single-instance default) alongside backend and frontend. Use `pnpm run rovodev -- 6` for full pool. `pnpm run dev` starts only backend + frontend (requires RovoDev Serve already running). Chat endpoints return 503 when RovoDev Serve is unavailable unless `AUTO_FALLBACK_TO_AI_GATEWAY=true` is set and AI Gateway config is valid.
