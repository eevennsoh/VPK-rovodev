...
Examples:
  ...
  python deploy_forge_app.py --app-dir ./my-forge-app --deploy-only
    ...
    if not args.deploy_only:
        ...
        try:
            install_app(app_dir, args.site, args.product, args.env)
        except Exception as e:
    ...
    print_success("Your Forge app is now deployed and installed!")
    print(f"\n{Colors.BOLD}Next steps:{Colors.ENDC}")
    print(f"  1. Go to {args.site}")
    print(f"  2. Open a Jira issue")
    ...
    print(f"  Redeploy:   python {sys.argv[0]} --app-dir {app_dir} --site {args.site} --skip-deps")
    print()
