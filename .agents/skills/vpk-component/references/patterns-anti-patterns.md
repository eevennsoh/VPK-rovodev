...
| Skipping computed style extraction for "obvious" properties | Even for properties like `border-radius` that seem predictable, always extract via `/agent-browser` — ADS CSS-in-JS compilation can produce unexpected values |
