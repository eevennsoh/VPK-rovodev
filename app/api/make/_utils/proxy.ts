...
function buildErrorMessage(rawText: string, requestPath?: string): string {
	...
	if (
		(requestPath?.startsWith("/api/make/runs") ?? false) &&
		/Cannot (GET|POST) \/api\/make\/runs/i.test(trimmedText)
	) {
		return "Backend is running without make run routes. Restart the backend dev server to load the latest code.";
	}
