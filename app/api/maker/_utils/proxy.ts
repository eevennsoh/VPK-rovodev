...
function buildErrorMessage(rawText: string, requestPath?: string): string {
	...
	if (
		(requestPath?.startsWith("/api/maker/runs") ?? false) &&
		/Cannot (GET|POST) \/api\/maker\/runs/i.test(trimmedText)
	) {
		return "Backend is running without maker run routes. Restart the backend dev server to load the latest code.";
	}
