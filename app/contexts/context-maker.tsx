...
import type { AgentRunListItem, AgentRun } from "@/lib/maker-run-types";
...
import type { TaskExecution } from "@/components/templates/maker/lib/execution-data";
import type { ExecutionState } from "@/components/templates/maker/hooks/use-execution-mode";
import type { ChatHistoryItem } from "@/components/templates/maker/components/sidebar-chat-history";
import type { SkillDialogProps, AgentDialogProps, SidebarConfigHandlers, ImportDialogState, DeleteAlertState } from "@/components/templates/maker/hooks/use-config-dialogs";
import type { RetryTaskGroupKey } from "@/components/templates/maker/lib/retry-task-groups";
import {
...
import {
	selectRetryTasks,
} from "@/components/templates/maker/lib/retry-task-groups";
import {
...
} from "@/components/templates/maker/lib/message-utils";
import { useMakerChat } from "@/components/templates/maker/hooks/use-maker-chat";
import { useExecutionMode } from "@/components/templates/maker/hooks/use-execution-mode";
import { useMakerConfig } from "@/components/templates/maker/hooks/use-maker-config";
import { useConfigDialogs } from "@/components/templates/maker/hooks/use-config-dialogs";
...
export function MakerProvider({ children }: MakerProviderProps) {
	...
	useEffect(() => {
		...
		const loadRunHistory = async () => {
			try {
				const response = await fetch(API_ENDPOINTS.makerRuns(), {
					cache: "no-store",
				});
	...
	const handleSelectRun = useCallback(
		(selectedRunId: string) => {
			router.push(`/maker/runs/${selectedRunId}`);
		},
		[router],
	);
		...
		async (deletedRunId: string) => {
			try {
				const response = await fetch(
					API_ENDPOINTS.makerRun(deletedRunId),
					{ method: "DELETE" },
				);
				if (!response.ok) {
	...
	const handleRetryRunGroup = useCallback(
		async (targetRunId: string, groupKey: RetryTaskGroupKey, taskIds: string[]) => {
			...
			try {
				const response = await fetch(API_ENDPOINTS.makerRunTasks(targetRunId), {
					method: "POST",
					headers: {
				...
				router.push(`/maker/runs/${targetRunId}`);
			} catch (error) {
				console.error(
...
export function useMaker(): MakerContextValue {
	const context = use(MakerContext);
	if (context === null) {
		throw new Error("usePlan must be used within a MakerProvider");
	}
	return context;
}

export function useMakerState(): MakerState {
	return useMaker().state;
}

export function useMakerActions(): MakerActions {
	return useMaker().actions;
}

export function useMakerMeta(): MakerMeta {
	return useMaker().meta;
}
