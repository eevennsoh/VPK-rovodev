...
import type { PlanApprovalSubmission } from "@/components/templates/shared/lib/plan-approval";
...
interface RovoChatPlanContextType {
	isOpen: boolean;
	toggleChat: () => void;
	closeChat: () => void;
...
const RovoChatPlanContext = createContext<RovoChatPlanContextType | undefined>(undefined);
...
export function RovoChatPlanProvider({ children }: { children: ReactNode }) {
...
export function useRovoChatPlan() {
	const context = use(RovoChatPlanContext);
	if (context === undefined) {
		throw new Error("useRovoChatPlan must be used within a RovoChatPlanProvider");
	}
	return context;
}
