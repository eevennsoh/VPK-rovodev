...
import { RunWorkspace } from "@/components/templates/maker/components/run-workspace";
...
function parseFetchFailure(error: unknown, fallbackMessage: string): string {
	...
	if (trimmedMessage.toLowerCase().includes("fetch failed")) {
		return "Unable to reach the maker run service. Check that the frontend and backend dev servers are running.";
	}
...
export default async function MakerRunSummaryPage({ params }: Readonly<RunSummaryPageProps>) {
	...
	const [runResult, summaryResult, filesResult] = await Promise.all([
		fetchJson<RunResponse>(
			`${requestOrigin}/api/maker/runs/${encodedRunId}`,
			"Failed to load run data."
		),
		fetchJson<RunSummaryResponse>(
			`${requestOrigin}/api/maker/runs/${encodedRunId}/summary`,
			"Failed to load run summary."
		),
		fetchJson<RunFilesResponse>(
			`${requestOrigin}/api/maker/runs/${encodedRunId}/files`,
