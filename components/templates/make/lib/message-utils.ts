...
function isHiddenPlanningSubmissionMessage(message: RovoUIMessage): boolean {
	...
	return (
		...
		message.metadata?.source === "plan-retry"
	);
}
...
export function isAnyWidgetCurrentlyLoading(
	...
	for (let index = messages.length - 1; index >= 0; index--) {
		...
		const loadingPart = getLatestDataPart(message, "data-widget-loading");
		if (loadingPart?.data.loading !== true) return false;
		// Question cards appear without a loading indicator
		return loadingPart.data.type !== "question-card";
	}
...
export function getLoadingWidgetType(
	...
	for (let index = messages.length - 1; index >= 0; index--) {
		...
		if (loadingPart?.data.loading === true) {
			const type = typeof loadingPart.data.type === "string" ? loadingPart.data.type : null;
			// Question cards appear without a loading indicator
			return type === "question-card" ? null : type;
		}
