...
import { isAgentRunStreamEvent } from "@/lib/maker-run-types";
import type { AgentExecutionUpdate } from "@/lib/rovo-ui-messages";
import { deriveTaskExecutionsFromRun } from "@/components/templates/maker/lib/execution-data";
import {
	applyExecutionUpdate,
	mergeStreamedExecutions,
	type TaskExecutionByTaskId,
} from "@/components/templates/maker/lib/task-execution-stream";
import { ExecutionGridView } from "@/components/templates/maker/components/execution-grid-view";
import { RunFilesTab } from "@/components/templates/maker/components/run-files-tab";
import { RunSummarySection } from "@/app/maker/runs/[runId]/run-summary-section";
import { AppSidebar } from "@/components/templates/maker/components/app-sidebar";
import ChatTitleRow from "@/components/templates/maker/components/chat-title-row";
...
import { ConfigDialogs } from "@/components/templates/maker/components/config-dialogs";
import {
	selectRetryTasks,
...
export function RunWorkspace({
	...
	const refreshSummary = useCallback(async () => {
		try {
			const response = await fetch(API_ENDPOINTS.makerRunSummary(runId), {
				cache: "no-store",
			});
			if (!response.ok) {
	...
	const refreshFiles = useCallback(async () => {
		...
		try {
			const response = await fetch(API_ENDPOINTS.makerRunFiles(runId), {
				cache: "no-store",
			});
			if (!response.ok) {
				const payload = (await response.json().catch(() => ({}))) as unknown;
	...
	useEffect(() => {
		...
		closeEventSource();
		const source = new EventSource(API_ENDPOINTS.makerRunStream(runId));
	...
	const postRunShare = useCallback(
		async (requestBody: AgentRunShareRequest): Promise<AgentRunShareResponse> => {
			const response = await fetch(API_ENDPOINTS.makerRunShare(runId), {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
				},
