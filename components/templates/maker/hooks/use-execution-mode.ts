...
export function useExecutionMode(): UseExecutionModeReturn {
	...
	const connectToRunStream = useCallback(
		(nextRunId: string) => {
			closeEventSource();
			const source = new EventSource(API_ENDPOINTS.makerRunStream(nextRunId));
	...
	const startExecution = useCallback(
		async (options: StartExecutionOptions) => {
			...
			const response = await fetch(API_ENDPOINTS.MAKER_RUNS, {
				method: "POST",
				headers: {
		...
		async (
			...
			try {
				const response = await fetch(
					API_ENDPOINTS.makerRunDirectives(targetRunId),
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
