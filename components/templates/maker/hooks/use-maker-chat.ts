...
import {
	MAKER_MODE_CONTEXT_DESCRIPTION,
	MAKER_MODE_POST_CLARIFICATION_CONTEXT_DESCRIPTION,
	MAKER_MODE_RETRY_PROMPT,
} from "../lib/maker-mode";
import type { ChatHistoryItem } from "../components/sidebar-chat-history";
import {
	MAKER_THREAD_RETENTION_LIMIT,
...
const MAKER_MODE_SOURCE = "plan-toggle" as const;
...
export function useMakerChat(): UsePlanChatReturn {
	...
	useEffect(() => {
		...
		let cancelled = false;
		fetch(API_ENDPOINTS.makerThreads())
			.then((response) => {
				if (!response.ok) {
		...
		(chatId: string | null, messages: ReadonlyArray<RovoUIMessage>) => {
			...
			setThreads((previousThreads) => {
				...
				return updateThreadMessages({
					...
					messages,
					updatedAt: Date.now(),
					maxThreads: MAKER_THREAD_RETENTION_LIMIT,
				});
			});
		...
		async (nextPrompt: string) => {
			...
			if (isPlanMode) {
				...
				await sendPrompt(nextPrompt, {
					contextDescription: MAKER_MODE_CONTEXT_DESCRIPTION,
					planMode: true,
	...
	useEffect(() => {
		...
		void sendPrompt(MAKER_MODE_RETRY_PROMPT, {
			contextDescription: MAKER_MODE_POST_CLARIFICATION_CONTEXT_DESCRIPTION,
			planMode: true,
			planModeSource: MAKER_MODE_SOURCE,
