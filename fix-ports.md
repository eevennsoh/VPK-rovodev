...
### 2026-02-22 - Destructive cancel probe causes port death during rapid message queueing
- **What happened:** When a user queued multiple messages (send msg1, msg2, msg3 in rapid succession), ports became permanently stuck. RovoDev Serve logged `Cancelling ongoing chat request` followed by `Chat did not finish within timeout during cancellation`.
- **Why:** `waitForPortReady()` in `backend/server.js` used `POST /v3/cancel` as a readiness probe after every port release. This is destructive — it triggers actual cancellation logic inside RovoDev Serve. After a normal stream completion, RovoDev may still be doing internal cleanup (session finalization, context saving). The cancel probe finds RovoDev in a transitional state, triggers real cancellation, RovoDev can't finish within the 3s timeout, the probe fails, and after 20 failed probes the pool marks the port unhealthy. This is amplified with queued messages because there are two cooldown+probe cycles per message (one for the main stream release, one for the suggestion generation release).
- **Fix:**
  - **Non-destructive readiness probe**: Replaced `rovoDevCancelChat(port, { timeoutMs: 3_000 })` with `rovoDevHealthCheck(port)` in `waitForPortReady()`. `GET /healthcheck` checks the server is alive without side effects — no state mutation, no cancellation.
  - **No retry on 409**: 409 ("chat already in progress") is terminal — the port never recovers on its own. Retrying is pointless. Keep single-attempt fail-fast: 409 → `ROVODEV_PORT_STUCK` → `restartRovoDevPort` (kills process via SIGTERM/SIGKILL, waits for replacement). This recovery path already existed and works.
  - **Tradeoff accepted**: The healthcheck probe is blind to turn state (only confirms process is alive). If RovoDev hasn't finished its turn by the time the healthcheck passes, the next message will get 409. But with the cancel probe, the port would die *silently* during the readiness probe with no restart. With the healthcheck probe, the port dies via 409 → fail-fast → automatic restart recovery. The restart path is preferable to silent death.
- **Rule:** Never use destructive operations (`POST /v3/cancel`) as readiness probes. Use non-destructive health checks and accept that 409 may still occur — the fail-fast + `restartRovoDevPort` recovery path handles it. 409 is always terminal; retrying on 409 is waste.
- **Verified:** Browser-tested with 4 scenarios on a live RovoDev Serve instance — all passed with zero port errors:
  1. Single message → response + suggestions
  2. 3 rapid queued messages (count 1-3, 4-6, 7-9) → all fired sequentially, all completed with suggestions
  3. Send while streaming (long paragraph + immediate follow-up) → both completed in order
  4. 4 rapid-fire short messages (A, B, C, D) → all completed with suggestions, no 409s
...
| `backend/server.js` | Pre-stream classifiers use `excludePinnedPorts: true`; `portIndexRequestTimestamps` map for staleness detection; suggestions use panel's `portIndex`; `generateTextViaGateway` accepts `excludePinnedPorts`; **`ROVODEV_PORT_STUCK` error handler** with user notification + immediate port restart; removed dead retry callback code (`emitConflictWaitStatus`, `retryOccurred`, `onRetry`/`onRetryProgress`); **`waitForPortReady()` uses non-destructive `rovoDevHealthCheck` instead of destructive `rovoDevCancelChat`** |
